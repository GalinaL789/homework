...
public class BankAccount {
   ...
   //    Логируйте операцию на уровне INFO, указывая владельца аккаунта и сумму пополнения.
    public boolean deposit(double amount) {
        if (balance > 0) {
            balance += amount;
            LOGGER.info("Баланс пополнен у владельца {}, на {}. Всего на счету {}.", owner, amount, balance);
            return true;
        } else {
    ...
    //Проверьте, достаточно ли средств на аккаунте. Если не достаточно, залогируйте это как ERROR,
    // указывая владельца аккаунта и требуемую сумму.
